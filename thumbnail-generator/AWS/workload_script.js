...
export default function() {
	const trace_header = getXrayTraceHeader()

	const image_to_use = randomIntBetween(0, num_different_images-1)

	const res = http.request('POST', `${base_url}/upload?filename=img-${trace_header.trace_id}.jpg`, image_data_array[image_to_use], {
		headers: {
			'Content-Type': 'image/png',
			'X-Amzn-Trace-Id': trace_header.header
		},
		tags: { // Each request is tagged in the metrics with the corresponding x-ray header
			xray_header: trace_header.header,
		}
	})

	// to debug
	// console.log(JSON.stringify(res))
	console.log(">>>>>>i am k6 in lambda:" + trace_header.header)
...
function getXrayTraceHeader() {
	...
	return {
        trace_id: trace_id,
        header: `Root=1-${current_time_hex}-${trace_id}`
    };
}
