...
class Provider:
    ...
    def login(self, aws_session_token=None):
        cmd = f"docker run --rm -it -v {self.volume_mount()} {self.cli_image()} {self.login_cmd()}"
        logging.info(cmd)
        os.system(cmd)
        # Workaround for temporary session token in AWS
        if aws_session_token and self.name == 'aws':
            set_token_cmd = f"aws configure set aws_session_token {aws_session_token}"
            session_cmd = f"docker run --rm -it -v {self.volume_mount()} {self.cli_image()} {set_token_cmd}"  # noqa: E501
            os.system(session_cmd)
