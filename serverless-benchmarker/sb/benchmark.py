...
class Benchmark:
    ...
    def prepare(self):
        ...
        self.save_config()
        logging.info(f"prepare_time={end - start}")
    ...
    def invoke(self, workload_type=None,
        ...
        self.save_config()
        logging.info(f"invoke_time={end - start}")
    ...
    def cleanup(self):
        ...
        self.remove_config()
        logging.info(f"cleanup_time={end - start}")
        logging.info(f"total_time={self.spec.event_log.total_duration()}")
    ...
    def load_spec(self, yml_config):
        ...
        if self.config_path().is_file():  # prior sb config exists
            ...
            if plugin_config[name] is None:
                ...
                # Deep merge: plugin config takes precedence over restored config
                config = merge({}, loaded_config, plugin_config)
